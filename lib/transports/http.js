...
class HTTPTransport {
  ...
  buildRequest (link, decoders, params = {}, customQueryParams = {}) {
    const fields = link.fields
    const method = link.method.toUpperCase()
    let queryParams = customQueryParams
    let pathParams = {}
    let formParams = {}
    let fieldNames = []
    let hasBody = false
  ...
  action (link, decoders, params = {}, customQueryParams = {}) {
    const responseCallback = this.responseCallback
    const request = this.buildRequest(link, decoders, params, customQueryParams)
